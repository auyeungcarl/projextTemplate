...
<%List list=(List)request.getAttribute("blogs");
	for(int i=0;i<list.size();i++){
		...
		String date=sdf.format(blog.getCreatedTimeToTimestemp());
		
		sdf=new SimpleDateFormat("HH:mm:ss");
		String time=sdf.format(blog.getCreatedTimeToTimestemp());
%>
   	<h2><%=date%></h2>
    <div class="entry">
	    <a id="6"></a>		
		...
		<p class="posted"><%=time %> <a href="<%=request.getContextPath()%>/blog/servlet/HomeServlet?cid=<%=blog.getCategoryId()%>"><%=blog.getCategory()%></a> | <a href="">评论</a></p>
    </div>
   	<%} %>
...
<div class="sidebar">
  ...
  <h2>分类</h2>
   <ul>		
	...
	%>
	    <li><a href="<%=request.getContextPath()%>/blog/servlet/HomeServlet?cid=<%=category.getId()%>"><%=category.getName()%></a></li>
     <%} %>  
   </ul>
		...
		<ul>	
			<%List recentBlogs=(List)request.getAttribute("blogs");
			  for(int i=0;i<recentBlogs.size();i++){
				...
				%>
	    		<li><a href="<%=request.getContextPath()%>/blog/servlet/HomeServlet?method=get&id=<%=blog.getId()%>" target="_blank"><%=blog.getTitle()%></a></li>
            <%}%>
      </ul>
	  ...
	  %>
	  	    <li><a href="<%=request.getContextPath()%>/blog/servlet/HomeServlet?method=get&id=<%=c.getBlogId()%>" target="_blank"><%=c.getContent()%></a></li>
        <%} %>        
      </ul>
