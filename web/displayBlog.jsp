...
<table id="tab">
  ...
  <tr>
    <td><%=blog.getCreatedTimeToTimestemp()%></td>
  </tr>
  ...
  for(int i=0;i<list.size();i++){ 
    ...
    <table id="tab">
      ...
      <tr>
        <td><%=comment.getCreatedTimeDate()%></td>
        </tr>
    </table>
    <br><br>
      <%}
		...
		<ul>	
			<%List recentBlogs=(List)request.getAttribute("blogs");
			  for(int i=0;i<recentBlogs.size();i++){
				...
				%>
	    		<li><a href="<%=request.getContextPath()%>/blog/servlet/HomeServlet?method=get&id=<%=blog2.getId()%>" target="_blank"><%=blog2.getTitle()%></a></li>
            <%}%>
      </ul>
